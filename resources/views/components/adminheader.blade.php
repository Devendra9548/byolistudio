<header>
    <div class="herosection d-flex align-items-center">
       <h2>Byoli Studio</h2>
    </div>
    <div class="container left-menus">
        <div class="row">
            <div class="col-12">
                <nav>
                    <ul>
                    <li><a href="{{ route('dashboard') }}"><i class="fa-solid fa-gauge"></i> Dashboard</a></li>
                    <li><a href="/admin/all-homes/1"><i class="fa fa-home" aria-hidden="true"></i>Home Page</a></li>
                    <li><a href="{{ route('all-blogs') }}"><i class="fa-solid fa-blog"></i> Blogs</a></li>
                    <li><a href="{{ route('all-products') }}"><i class="fa-solid fa-blog"></i> Products</a></li>
                    <li><a href="{{ route('all-orders') }}"><i class="fa-solid fa-message"></i> Orders</a></li>
                    <li><a href="{{ route('all-reviews') }}"><i class="fa-solid fa-ranking-star"></i> Reviews</a></li>
                    <li><a href="{{ route('all-customers') }}"><i class="fa-solid fa-users"></i> Customers</a></li>
                    <li><a href="{{ route('all-contacts') }}"><i class="fa-solid fa-address-card"></i> Contacts</a></li>
                    <li><a href="{{ route('page-seo') }}"><i class="fa-solid fa-book-atlas"></i> Pages Seo</a></li>
                    <li><a href="{{ route('global-seo') }}"><i class="fa-solid fa-globe"></i> Global Seo</a></li>
                    <li><a href="{{ route('admin-info') }}"><i class="fa-solid fa-circle-user"></i> Admin Info</a></li>
                    </ul>
                </nav>
            </div>
        </div>
    </div>
</header>