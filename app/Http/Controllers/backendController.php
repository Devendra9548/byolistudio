...
function allproducts(Request $req){
  ...
  if(Cookie::has('pswd')){
    if($search != ''){
      ...
      $dbs = Product::orderBy('id', 'desc')->get();
...
function allorders(){
  if(Cookie::has('pswd')){
        ...
        ->select('orders.orderid as orderid', 'customers.username as username', 'orders.amount', 'orders.status', 'orders.payment_id','orders.paymethod','orders.paystatus', 'orders.rzp_order_id', 'orders.created_at')
...
public function allreviews(Request $req) {
    if (Cookie::has('pswd')) {
...
function allhomes($id){

  if(Cookie::has('pswd')){
    $dbs = HomePage::find($id);
    return view('admin.home-page.all-home',['members'=>$dbs]);
  }
  else{
    return redirect('admin');
  }
...
function UpdateHome(Request $req)
{
  if(Cookie::has('pswd')){
  $origname='';
  $dbs = HomePage::find($req->id);
  ...
  if (!$dbs) {
    ...
    // Check if a new image is uploaded for updating
  if ($req->hasFile('slide1')) {
    $image = $req->file('slide1');
    $name = $image->getClientOriginalName();
    $t=time();
    $d=date("Y-m-d",$t);
    $origname = $d."-".$t."-".$name;
  ...
  if ($req->hasFile('slide2')) {
    $image = $req->file('slide2');
    $name = $image->getClientOriginalName();
    $t=time();
    $d=date("Y-m-d",$t);
    $origname = $d."-".$t."-".$name;
  ...
  if ($req->hasFile('slide3')) {
    $image = $req->file('slide3');
    $name = $image->getClientOriginalName();
    $t=time();
    $d=date("Y-m-d",$t);
    $origname = $d."-".$t."-".$name;
  ...
  if ($req->hasFile('slide4')) {
    $image = $req->file('slide4');
    $name = $image->getClientOriginalName();
    $t=time();
    $d=date("Y-m-d",$t);
    $origname = $d."-".$t."-".$name;
  ...
  if ($req->hasFile('slide5')) {
    $image = $req->file('slide5');
    $name = $image->getClientOriginalName();
    $t=time();
    $d=date("Y-m-d",$t);
    $origname = $d."-".$t."-".$name;
  ...
  if ($req->hasFile('slide6')) {
    $image = $req->file('slide6');
    $name = $image->getClientOriginalName();
    $t=time();
    $d=date("Y-m-d",$t);
    $origname = $d."-".$t."-".$name;
  ...
  if ($req->hasFile('slide7')) {
    $image = $req->file('slide7');
    $name = $image->getClientOriginalName();
    $t=time();
    $d=date("Y-m-d",$t);
    $origname = $d."-".$t."-".$name;
